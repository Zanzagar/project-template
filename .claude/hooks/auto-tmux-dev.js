...
process.stdin.on('end', () => {
  ...
  try {
    ...
    // Detect dev server commands (ECC Node.js + template polyglot patterns)
    // Use word boundary (\b) to avoid matching partial commands
    const devServerRegex = /(npm run dev\b|pnpm( run)? dev\b|yarn dev\b|bun run dev\b|flask run\b|uvicorn\b|python -m uvicorn\b|hugo serve\b|jekyll serve\b|cargo watch\b)/;
