...
const DEV_COMMAND_WORDS = new Set([
  ...
  'npx',
  'tmux',
  // Polyglot server commands (template addition)
  'flask',
  'uvicorn',
  'python',
  'python3',
...
process.stdin.on('end', () => {
  try {
    ...
    if (process.platform !== 'win32') {
      ...
      const devPattern = /\b(npm\s+run\s+dev|pnpm(?:\s+run)?\s+dev|yarn\s+dev|bun\s+run\s+dev|flask\s+run|uvicorn\s+|python3?\s+(-m\s+)?uvicorn|hugo\s+serve|jekyll\s+serve|cargo\s+watch)\b/;
