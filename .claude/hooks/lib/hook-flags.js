 ...
 * Adapted from ECC (affaan-m/everything-claude-code) scripts/lib/hook-flags.js
 *
 * Controls:
 * - TEMPLATE_HOOK_PROFILE=minimal|standard|strict (default: standard)
 * - TEMPLATE_DISABLED_HOOKS=comma,separated,hook,ids
 */
...
function getHookProfile() {
  const raw = String(process.env.TEMPLATE_HOOK_PROFILE || 'standard').trim().toLowerCase();
  return VALID_PROFILES.has(raw) ? raw : 'standard';
}

function getDisabledHookIds() {
  const raw = String(process.env.TEMPLATE_DISABLED_HOOKS || '');
