...
/**
 * Get the global sessions directory (~/.claude/sessions/)
 */
...
/**
 * Get the project-local sessions directory (.claude/sessions/ relative to git root)
 * Falls back to global sessions dir if not in a git repository
 */
function getProjectSessionsDir() {
  const result = runCommand('git rev-parse --show-toplevel');
  if (result.success) {
    return path.join(result.output, '.claude', 'sessions');
  }
