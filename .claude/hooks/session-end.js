...
function buildSummarySection(summary) {
  ...
  // Task Master in-progress tasks (template addition)
  const taskResult = runCommand('task-master list --status in-progress --json');
  if (taskResult.success) {
    try {
      const tasks = JSON.parse(taskResult.output);
      const taskList = (tasks.tasks || []).slice(0, 10);
      if (taskList.length > 0) {
        section += '### Task Master (In Progress)\n';
        for (const t of taskList) {
          section += `- ${t.title || t.name || 'Untitled'} (ID: ${t.id})\n`;
        }
        section += '\n';
      }
    } catch {
      // task-master output not valid JSON, skip
    }
  }
