...
When the project uses `github.com/stretchr/testify`, prefer its assertion helpers for clearer failure messages:
...
**Rule of thumb**: Use `require` for preconditions (nil checks, error checks that would cause a panic if continued), `assert` for everything else.
