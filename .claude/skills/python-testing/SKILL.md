...
Use factory fixtures when tests need multiple instances with varying parameters:
...
def test_multiple_users(make_user):
    admin = make_user(name="admin", email="admin@example.com")
    guest = make_user(name="guest")
    assert admin.email != guest.email
```
...
1. **Dependency injection** — design the function to accept its dependencies as parameters. Zero coupling to mock infrastructure.
2. **`patch` as context manager** — when you can't change the interface. Scoped to the `with` block.
3. **`@patch` decorator** — when the same mock is needed for the full test. Avoid stacking more than 2-3 decorators.
...
| `@pytest.fixture` (factory) | Return a callable for multiple instances |
| `@pytest.mark.parametrize()` | Run tests with multiple inputs |
| `@pytest.mark.slow` | Mark slow tests |
| `pytest -m "not slow"` | Skip slow tests |
| `@patch()` | Mock functions and classes (when DI isn't possible) |
