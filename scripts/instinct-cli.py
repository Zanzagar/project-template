...
Modified for project-scoped instinct storage (.claude/instincts/) instead
of global ~/.claude/homunculus/.
...
INSTINCTS_DIR = PROJECT_ROOT / ".claude" / "instincts"
...
def load_candidates() -> list[dict]:
    """Load session candidates from candidates/ directory."""
    candidates = []
    if not CANDIDATES_DIR.exists():
        return candidates

    for file in sorted(CANDIDATES_DIR.glob("*.json")):
        try:
...
def cmd_status(args):
    ...
    # Show candidates
    if candidates:
        print(f"  Session candidates: {len(candidates)}")
        for cand in candidates[-5:]:  # Show latest 5
            session = cand.get("session_id", "unknown")
            commits = cand.get("commit_count", 0)
            category = cand.get("detected_category", "general")
            print(f"    - {session}: {commits} commits ({category})")
        print()
...
def cmd_evolve(args):
    ...
    # High-confidence instincts by domain
